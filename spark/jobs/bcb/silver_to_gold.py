...
def read_series(
    ...
    if not path_exists(spark, silver_path):
        print(
            f"Silver path not found: {silver_path}"
        )
        return None
...
def write_gold(
    ...
    print(
        f"Writing Gold Delta table: {gold_path}"
    )
...
def main():
    ...
    try:
        ...
        if gold_df is None:
            print(
                "No data available to build Gold."
            )
            return
